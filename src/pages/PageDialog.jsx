...
const PageDialog = ({
    ...
    const handleOptionClick = async (option, index) => {
        setDisabledOption(index); // Simpan indeks tombol yang diklik
        ...
        <div className="justify-center items-center">
            ...
            >
                <div className="h-fit relative flex justify-evenly items-center">
                    {Array.isArray(gambarkarakter) &&
                        gambarkarakter.map((src, index) => (
                            <img
                                className="w-96"
                                key={index}
                                src={src}
                ...
                >
                    ...
                    <div className="relative z-10 flex gap-10 comic ">
                        {showButtons &&
                            opsi &&
                            opsi.length > 0 &&
                            opsi.map((option, index) => (
                                <button
                                    key={index}
                                    className={`button-56 button-fade-in mt-5 ${disabledOption !== null && disabledOption !== index
                                            ? "button-hidden" // Sembunyikan tombol lain
                                            : disabledOption === index
                                                ? "button-disabled" // Ubah warna tombol yang diklik
                                                : ""
                                        }`}
                                    role="button"
                                    onClick={() => handleOptionClick(option, index)}
                                    disabled={disabledOption !== null} // Nonaktifkan semua tombol setelah klik
                                >
                                    {option.text}
                                </button>
                            ))}
